...
Ideally the system will be able to self regulate its users, for example if a customer as been inactive for an extended period of time the system will be able to automatically remove that user's information from the database thus freeing up space for other potential customers.

Additionally employees are concerned if the guidelines and scripts provided by the system is adaptive enough to cater for every customer as well as the accuracy of the grading system as there are specific scenarios where the grading system might be unfair towards the employee.
...
### System Flow
![](./activity%20diagram%20for%20call%20centre%20management%20in%20travel%20company.JPG)
This diagram shows how our system will handle both inbound and outbound calls simultaneously. Not only will the system manage inbound customer calls divertly potential customers to the appropriate RM or to the IVR, it will also generate a customer profile based to the information collected by the IVR and assign that call to the most appropriate RM based on their profile.
## Scrum
...
By categorising these customers the system is able to match them with the next avaliable and best suited RM which reduces overall wait time and the chances of customers dropping out of queues due to these long wait times.
