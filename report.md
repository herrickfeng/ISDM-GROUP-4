...
The system will have three major components. A database where customer/company profiles and data can be securely stored and accessed. An inbound/outbound call management component where calls will divert to the most appropriate RM based on the customer profile and the RM profile. Lastly, it will also contain a management app where company managers can access and review the performances of their team/s.

## Objectives

- Improve Customer Experience
- Increase CMC Efficiency
- Monitor and Review RM Performance

## Problem Definition

The current system has inbound customer calls diverted to any available RM without regard to expertise or experience. This creates a bottleneck when a customer with a specific request is paired with an RM that doesn't have the requisite expertise/experience. Resulting in an increase in the customer waiting time as well as a decrease in customer satisfaction.

On the outbound end, currently, an RM would blindly call a customer without any knowledge of their needs or interests, this generates issues as that specific RM may not have the necessary knowledge or language to properly communicate with the potential customer, resulting in a decrease of sales efficiency.

The system must also be able to catch any customers that have been waiting for an extended amount of time and direct to the Interactive Voice Response. Finally, as of now, Managers do not have a concrete grading system to review the performance of their team/s, this results in a decrease of efficiency in the CMC as manages cannot correctly identify the RMs that have a lower sales rate.
...
![Outbound Customers](./EMPATHY_MAP/outbound-customer-empathy-map.png)

#### 2. Inbound Customers

![Inbound Customers](./EMPATHY_MAP/Empathy%20Map.png)

#### 3. Board Members

![Board Members](./EMPATHY_MAP/board%20members.JPG)

#### 4. Company Managers

![Company Manager](./EMPATHY_MAP/Company%20Manager%20Empathy%20Map.jpg)

#### 5. Employees/ Potential Employees

![Employees](./EMPATHY_MAP/employees.JPG)

### Design Considerations

Interpreting the Point of views generated by the empathy maps, it is clear that the system, on top of the three main components, will require two additional components, a database that can securely store the company and customer information and an administration component that can monitor the users on the system and be alerted to any security breaches.

Ideally, the system will be able to self regulate its users, for example, if a customer has been inactive for an extended period the system will be able to automatically remove that user's information from the database thus freeing up space for other potential customers.

Additionally, employees are concerned if the guidelines and scripts provided by the system are adaptive enough to cater for every customer as well as the accuracy of the grading system as there are specific scenarios where the grading system might be unfair towards the employee.

### Database Needs

![](./MODELS/class%20diagram.JPG)

As detailed in the diagram above, our database must be robust and be able to handle a large about of company and customer data. As most of this information will be sensitive such as Names, Date of Births or Contact Information, therefore our database must have a high level of security.

### System Flow

![](./MODELS/activity%20diagram%20for%20call%20centre%20management%20in%20travel%20company.JPG)

This diagram shows how our system will handle both inbound and outbound calls simultaneously. Not only will the system manage the inbound customer calls, it will divert potential customers to the appropriate RM or the IVR, it will also generate a customer profile based to the information collected by the IVR and assign that call to the most appropriate RM based on their profile.
...
1. Employees/Potential employee's empathy towards customers:
   It was not specified within the specifications however it would be assumed that Relationship Managers would be empathetic towards customers so as to provide better customer service and understand the needs of their clients more wholly.
...
For each stakeholder that we recognised, we have created user stories. These can be found in the `USES_STORES` folder, and more specifically, in [this file](./USER_STORIES/user_stories.md).

To further understand the problem, we generated two use cases. One was created from the company manager's perspective, which can be found [here](./USE_CASES/UC201.md), and a second was created from the Employee's perspective, which can be found [here](./USE_CASES/UC301.md).

## Competitive Advantages

The implemented information system will result in multiple competitive advantages by strengthening factors such as productivity and revenue whilst also reducing disadvantages such as operational costs. These costs are addressed by automating multiple tedious tasks in which the system does by using a 'skill level' measurement relying on age, sex, culture and language proficiency which is all calculated by the Profiler Tool.

Furthermore, customers are measured by a score of 1-10 depending on preloaded criteria allowing the system to successfully match each customer to a suitable Relationship manager (RM). This is prominent in inbound calls as higher scored customers are served first preferably to higher-skilled RMs. Thus, by capitalising customers with a higher chance of purchase to the most proficient RMs will result in a much higher chance of revenue and success.
...
Thus, system failure would compromise the entire business as the simple loss of call flow control itself would delay the system substantially whilst creating more disorder and confusion between RMs and end-customers.
